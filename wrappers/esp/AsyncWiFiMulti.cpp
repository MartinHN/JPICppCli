...
uint8_t AsyncWiFiMulti::run(uint32_t connectTimeout) {
  ...
  status = handleScanDone(connectTimeout);
  return status;
}

uint8_t AsyncWiFiMulti::handleScanDone(uint32_t connectTimeout) {
  auto scanResult = WiFi.scanComplete();
  if (scanResult == WIFI_SCAN_FAILED) { // not started
    log_i("[WIFI] restart scan");
    WiFi.scanNetworks(true);
    return WL_NO_SSID_AVAIL;
  }
  if (scanResult == WIFI_SCAN_RUNNING) { // still running
    return WL_NO_SSID_AVAIL;
  }
  ...
  if (scanResult >= 0) {
    ...
    if (scanResult == 0) {
      ...
      for (int8_t i = 0; i < scanResult; ++i) {
        ...
        if (known) {
          log_d(" --->   %d: [%d][%02X:%02X:%02X:%02X:%02X:%02X] %s (%d) %c", i, chan_scan, BSSID_scan[0], BSSID_scan[1], BSSID_scan[2],
                BSSID_scan[3], BSSID_scan[4], BSSID_scan[5], ssid_scan.c_str(), rssi_scan, (sec_scan == WIFI_AUTH_OPEN) ? ' ' : '*');
        } else {
          log_d("       %d: [%d][%02X:%02X:%02X:%02X:%02X:%02X] %s (%d) %c", i, chan_scan, BSSID_scan[0], BSSID_scan[1], BSSID_scan[2], BSSID_scan[3],
                BSSID_scan[4], BSSID_scan[5], ssid_scan.c_str(), rssi_scan, (sec_scan == WIFI_AUTH_OPEN) ? ' ' : '*');
        }
      }
    }
    ...
    if (bestNetwork.ssid) {
      log_i("[WIFI] Connecting BSSID: %02X:%02X:%02X:%02X:%02X:%02X SSID: %s Channel: %d (%d)", bestBSSID[0], bestBSSID[1], bestBSSID[2],
            bestBSSID[3], bestBSSID[4], bestBSSID[5], bestNetwork.ssid, bestChannel, bestNetworkDb);

      WiFi.begin(bestNetwork.ssid, bestNetwork.passphrase, bestChannel, bestBSSID);
      status = connectTimeout > 0 ? WiFi.status() : WL_SCAN_COMPLETED;
...
bool AsyncWiFiMulti::hasHandledResultAndClear() {
  if (hasHandledResult) {
